...
export default function PortfolioManager({ portfolio = [], csrf, onDone, onError }) {
  ...
  const upload = async () => {
    ...
    setBusy(true)
    try { await api.uploadPortfolioImages(draft.id, files, draft.version, csrf); setFiles([]); await onDone() } catch (error) { onError(error) } finally { setBusy(false) }
  }
