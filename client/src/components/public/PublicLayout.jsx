...
export default function PublicLayout({ children }) {
  ...
  useEffect(() => {
    ...
    document.title = `${titles[pathname] || (pathname.endsWith('/payment') ? 'Thanh toán demo' : 'Không tìm thấy trang')} — Ban Mai`
  }, [pathname])
  useEffect(() => {
    if (!open) return
    const onKey = (event) => {
  ...
  return <div className="app-shell">
    ...
    <header className="header-wrap"><div className="app-header">
      ...
      <nav ref={nav} id="primary-navigation" aria-label="Điều hướng chính" className={open ? 'is-open' : ''}>
        ...
        <a href="#studio-contact" onClick={(event) => { event.preventDefault(); setOpen(false); requestAnimationFrame(() => { const contact = document.getElementById('studio-contact'); contact?.focus({ preventScroll: true }); contact?.scrollIntoView({ behavior: 'instant', block: 'start' }) }) }}>Liên hệ</a>
