...
const channels = [
  { id: 'tiktok', name: 'TikTok', label: 'Xem TikTok Studio', note: 'Xem thêm những khoảnh khắc, concept và hình ảnh mới nhất.' },
...
export default function StudioContact() {
  ...
  useEffect(() => {
    ...
    fetch('/studio-contact.json', { signal: abort.signal, cache: 'no-store' })
      ...
      .then((data) => { if (!abort.signal.aborted) setLinks({ zalo: socialUrl('zalo', data?.zalo), facebook: socialUrl('facebook', data?.facebook), tiktok: socialUrl('tiktok', data?.tiktok) }) })
  ...
  return <section id="studio-contact" className="studio-contact" tabIndex="-1" aria-labelledby="contact-title">
    ...
    </div>)}<p className="contact-privacy">Liên kết mở trực tiếp trên TikTok, Zalo hoặc Facebook. Trang không nhúng widget theo dõi hay yêu cầu quyền truy cập tài khoản.</p></div>
  </section>
}
