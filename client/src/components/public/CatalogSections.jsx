...
export function PortfolioGrid({ items }) {
  return <div className="editorial-grid">{items.map((item, index) => <Link className={`editorial-card editorial-${index % 3}`} to={`/portfolio?concept=${encodeURIComponent(item.id)}`} key={item.id}>
    <div className="editorial-image"><StudioImage src={item.image} alt={item.name} sizes="(max-width: 700px) 88vw, 30vw" loading="lazy" decoding="async" width="600" height="800" /><span className="image-open" aria-hidden="true">↗</span></div>
...
export function CatalogNotice({ offline }) {
  return offline ? <p className="catalog-notice" role="status">Chưa tải được gói chụp. Vui lòng thử lại để xem thông tin mới nhất từ Studio.</p> : null
}
