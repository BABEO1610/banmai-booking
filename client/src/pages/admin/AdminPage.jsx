...
export default function AdminPage() {
  ...
  return <PublicLayout workspace><main className="admin-page admin-workspace"><Suspense fallback={<LoadingState />}>
    ...
    {!loading && tab === 'home' && <section className="admin-section"><HomeContentEditor contents={data.catalog?.contents?.filter((item) => ['home_intro', 'home_light_main', 'home_light_detail'].includes(item.key))} csrf={csrf} onDone={async () => { setMessage('Đã lưu nội dung trang chủ.'); await refresh() }} onError={(err) => setError(err.message)} /></section>}
