...
const upload = multer({ storage: multer.memoryStorage(), limits: { files: 1, fileSize: 10 * 1024 * 1024, fields: 1, fieldSize: 1024, parts: 2 }, fileFilter: (_req, _file, callback) => callback(null, true) })
const uploadImages = (request, response, next) => upload.array('images', 1)(request, response, (error) => error ? next(error.code?.startsWith('LIMIT_') ? new AppError(400, 'INVALID_IMAGE', 'Mỗi lần tải một ảnh, tối đa 10MB') : error) : next())
...
if (config.nodeEnv !== 'production') router.get('/v1/demo/mail', asyncHandler(async (request, response) => { if (authService.usingDatabase) throw new AppError(404, 'NOT_FOUND', 'Hộp thư demo không được bật khi dùng email thật'); return ok(response, demoStore.mail(request.query.email, request.get('x-demo-mail-token') || request.query.token)) }))
...
router.get('/v1/bookings', requireAuth, asyncHandler(async (request, response) => { const found = user(request); ok(response, paginate(demoStore.state.bookings.filter(b => found.role === 'ADMIN' || b.customerId === found.id).sort((a, b) => new Date(b.startAt) - new Date(a.startAt)), request.query, b => demoStore.bookingView(b, found.id, found.role === 'ADMIN'))) }))
...
router.get('/v1/admin/bookings', requireRole('ADMIN'), asyncHandler(async (request, response) => { const query = String(request.query.q || '').trim().toLowerCase(); const status = request.query.status; const photographerId = request.query.photographerId; const from = request.query.from ? new Date(request.query.from) : null; const to = request.query.to ? new Date(request.query.to) : null; const all = demoStore.state.bookings.filter((b) => { const owner = demoStore.userById(b.customerId); const assignment = b.assignmentId ? demoStore.state.assignments.find((a) => a.id === b.assignmentId) : null; const haystack = `${b.code} ${owner?.name || ''} ${owner?.email || ''} ${b.contact?.phone || ''}`.toLowerCase(); return (!query || haystack.includes(query)) && (!status || b.status === status) && (!photographerId || assignment?.photographerId === photographerId) && (!from || (request.query.overlap === 'true' ? new Date(b.endAt) > from : new Date(b.startAt) >= from)) && (!to || new Date(b.startAt) <= to) }).sort((a, b) => new Date(a.startAt) - new Date(b.startAt)); ok(response, paginate(all, request.query, b => demoStore.bookingView(b, null, true))) }))
...
router.post('/v1/admin/integrations/:target/retries', requireRole('ADMIN'), csrfGuard, asyncHandler(async (request, response) => { await demoStore.mutate(async () => { const events = demoStore.state.outbox.filter((item) => item.channel === request.params.target && item.status === 'RETRY'); events.forEach((item) => { item.status = 'PENDING'; item.availableAt = new Date().toISOString() }); }); ok(response, { target: request.params.target, status: 'QUEUED' }) }))
...
if (config.nodeEnv !== 'production') router.post('/v1/demo/payment/:id', requireRole('ADMIN'), csrfGuard, asyncHandler(async (request, response) => ok(response, await demoStore.simulatePayment(request.params.id, request.session.userId, request.body.amountVnd, request.body.transactionId))))
...
router.patch('/v1/admin/content/:id', requireRole('ADMIN'), csrfGuard, asyncHandler(async (request, response) => ok(response, await demoStore.updateContent(request.params.id, request.body || {}, request.session.userId, request.body?.version))))
router.post('/v1/admin/content/:id/image', requireRole('ADMIN'), csrfGuard, uploadBudget, uploadImages, asyncHandler(async (request, response) => { const file = request.files?.[0]; if (!file || !validImageBytes(file)) throw new AppError(400, 'INVALID_IMAGE', 'Chọn một ảnh JPEG, PNG hoặc WebP hợp lệ'); const uploaded = await uploadPublicImage({ bytes: file.buffer, prefix: `content/${request.params.id}`, localDir: mediaDir }); ok(response, await demoStore.updateContent(request.params.id, { image: uploaded.image }, request.session.userId, request.body?.version)) }))
