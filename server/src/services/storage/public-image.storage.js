...
async function ensureBucket() {
  ...
  const existing = await fetch(`${base}/storage/v1/bucket/${encodeURIComponent(config.storageBucket)}`, { signal: AbortSignal.timeout(15000), headers: { authorization: `Bearer ${config.supabaseServiceRoleKey}`, apikey: config.supabaseServiceRoleKey } })
  if (existing.ok) {
    const update = await fetch(`${base}/storage/v1/bucket/${encodeURIComponent(config.storageBucket)}`, {
      signal: AbortSignal.timeout(15000), method: 'PUT', headers: { authorization: `Bearer ${config.supabaseServiceRoleKey}`, apikey: config.supabaseServiceRoleKey, 'content-type': 'application/json' }, body: JSON.stringify({ public: true }),
    })
  ...
  const response = await fetch(`${config.supabaseUrl.replace(/\/$/, '')}/storage/v1/bucket`, {
    signal: AbortSignal.timeout(15000), method: 'POST', headers: { authorization: `Bearer ${config.supabaseServiceRoleKey}`, apikey: config.supabaseServiceRoleKey, 'content-type': 'application/json' },
...
export async function uploadPublicImage({ bytes, prefix = 'images', localDir }) {
  let extension = detectImageExtension(bytes)
  if (!extension) throw new AppError(400, 'INVALID_IMAGE', 'File không phải ảnh JPEG, PNG hoặc WebP hợp lệ')
  try {
    bytes = await sharp(bytes, { limitInputPixels: 25_000_000, animated: false }).rotate().resize({ width: 2000, height: 2000, fit: 'inside', withoutEnlargement: true }).webp({ quality: 82 }).toBuffer()
    extension = 'webp'
  } catch { throw new AppError(400, 'INVALID_IMAGE', 'Ảnh không đọc được hoặc vượt quá 25 megapixel') }
  ...
  if (config.imageStorage === 'supabase') {
    ...
    const response = await fetch(`${config.supabaseUrl.replace(/\/$/, '')}/storage/v1/object/${encodeURIComponent(config.storageBucket)}/${name.split('/').map(encodeURIComponent).join('/')}`, {
      signal: AbortSignal.timeout(15000), method: 'POST', headers: { authorization: `Bearer ${config.supabaseServiceRoleKey}`, apikey: config.supabaseServiceRoleKey, 'content-type': mimeByExtension[extension], 'cache-control': '31536000', 'x-upsert': 'false' }, body: bytes,
    })
